...
uint8_t code_pirata(char pos, char tipo)
{
    ...
    {
        Pirata_ClearRxBuffer();
        Pirata_ClearTxBuffer();    
        borrar();
    ...
    else if(bandera_entrada == 1 &&  bandera_posicion == pos)
    {
        ...
        if(Pirata_GetRxBufferSize() > 8 )
        {
	        x=0;		
            
            while(Pirata_GetRxBufferSize() > 0 && x < 8)
            {
           		buffer_RX[x] = Pirata_ReadRxData();
           		x++;
                CyDelay(1);                
        	}
            ...
            if(buffer_RX[7] > 0)
            {
                while(Pirata_GetRxBufferSize() > 0)
                {
           		    buffer_RX[x] = Pirata_ReadRxData();
           		    x++;
                    CyDelay(1); 
